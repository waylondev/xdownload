        ...
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <div className="w-full md:w-48">
            <Select value={selectedPlatform} onValueChange={onPlatformChange}>
              <SelectTrigger className="w-full bg-slate-800/80 backdrop-blur-sm border border-slate-700 text-slate-100 hover:bg-slate-700/80 transition-all duration-300 rounded-xl h-12 text-base shadow-lg hover:shadow-blue-500/20">
          ...
          <div className="flex-1 flex gap-3">
            ...
            <div className="relative flex-1">
              <div className="absolute inset-0 bg-gradient-to-r from-blue-600/20 to-purple-600/20 rounded-xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>
              <Input
                value={searchQuery}
                onChange={(e) => onSearchQueryChange(e.target.value)}
                onKeyPress={handleKeyPress}
                placeholder={`搜索${downloadTypes.find(t => t.id === activeType)?.name}...`}
                className="w-full bg-slate-800/80 backdrop-blur-sm border border-slate-700 text-slate-100 placeholder:text-slate-500 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all duration-300 rounded-xl h-12 text-base shadow-lg hover:shadow-blue-500/20 pl-5 pr-12"
              />
            ...
            <Button 
              ...
              className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white border-0 rounded-xl h-12 px-6 transition-all duration-300 shadow-lg hover:shadow-blue-500/30 hover:scale-105 disabled:opacity-50 disabled:pointer-events-none disabled:scale-100 group"
            >
              <div className="flex items-center gap-3">
                {loading ? (
